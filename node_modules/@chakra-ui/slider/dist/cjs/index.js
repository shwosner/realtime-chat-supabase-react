"use strict";

exports.__esModule = true;

var _rangeSlider = require("./range-slider");

Object.keys(_rangeSlider).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _rangeSlider[key]) return;
  exports[key] = _rangeSlider[key];
});

var _slider = require("./slider");

Object.keys(_slider).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _slider[key]) return;
  exports[key] = _slider[key];
});

var _useRangeSlider = require("./use-range-slider");

Object.keys(_useRangeSlider).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _useRangeSlider[key]) return;
  exports[key] = _useRangeSlider[key];
});

var _useSlider = require("./use-slider");

Object.keys(_useSlider).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _useSlider[key]) return;
  exports[key] = _useSlider[key];
});
//# sourceMappingURL=index.js.map